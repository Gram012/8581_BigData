...
def plot_run(name, version=None):
    if version is None:
    ...
    p = make_figure(
        x_axis_label="Step", y_axis_label="Train Loss", y_range=y_range, title=f"Version {version}"
    )
...
def plot_evaluation(**results):
    ...
    for color, (label, result) in zip(palette[2:], results.items()):
        fars, svs = evaluate(*result, max_far_per_week=1000)
        combos = sorted(svs)[::-1]
        if not plots:
